    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc='Process | Predict | Looping over batches'):
            ...
            for sampleNumber in range(batch_size):
                ...
                if out_labels_rows:
                    rows = np.sort(np.column_stack((separators_row_wide[:-1, 1], separators_row_wide[1:, 0]+1)), axis=1)
                    cols = np.sort(np.column_stack((separators_col_wide[:-1, 1], separators_col_wide[1:, 0]+1)), axis=1)
